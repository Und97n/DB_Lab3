class Controller(object):
    ...
    def show_table_menu(self, table_name):
        ...
        if input_v == '1':
            ...
            self.view.after_action_message(result)
            ...
            self.view.after_action_message(self.model.insert(db_object))
            self.show_table_menu(table_name)
        elif input_v == '4':
            filled_object = self.view.request_input_object(table_object.get_columns())
            updated_object = self.view.request_input_object(table_object.get_columns())
            self.view.after_action_message(self.model.update(table_name, filled_object, updated_object))
            self.show_table_menu(table_name)
        elif input_v == '5':
            filled_object = self.view.request_input_object(table_object.get_columns())
            self.view.after_action_message(self.model.delete(table_name, filled_object))
            self.show_table_menu(table_name)
        elif input_v == 'back':
            self.start()
